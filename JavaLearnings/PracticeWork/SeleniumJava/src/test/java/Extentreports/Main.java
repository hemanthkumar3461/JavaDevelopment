 ...
 * ExtentHtmlReporter("target/Spark/Spark.html"); extent.attachReporter(spark);
 * 
 * extent.createTest("ScreenCapture") .addScreenCaptureFromPath("extent.png")
 * .pass((Markup)
 * MediaEntityBuilder.createScreenCaptureFromPath("extent.png").build());
 * 
 * extent.createTest("LogLevels") .info("info") .pass("pass") .warning("warn")
 * .skip("skip") .fail("fail");
 * 
 * extent.createTest("CodeBlock").createLog( Status.PASS,
 * MarkupHelper.createCodeBlock(CODE1));
 * 
