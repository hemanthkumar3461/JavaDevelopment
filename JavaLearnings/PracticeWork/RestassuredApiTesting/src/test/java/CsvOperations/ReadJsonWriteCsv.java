 ...
 * 
 * import java.io.File; import com.fasterxml.jackson.databind.JsonNode; import
 * com.fasterxml.jackson.databind.ObjectMapper; import
 * com.fasterxml.jackson.dataformat.csv.CsvMapper; import
 * com.fasterxml.jackson.dataformat.csv.CsvSchema; import
 * com.github.dockerjava.transport.DockerHttpClient.Response; //Csv is also
 * format similar to Json //We need to add java jackson library in order to work
 * with serialization
 * 
 * we'll use a combination of ObjectMapper and CSVMapper to convert between JSON
 * and CSV. [ObjectMapper is the main actor class of Jackson library.
 * ObjectMapper class ObjectMapper provides functionality for reading and
 * writing JSON]
 * 
 ...
 * 
 * Response response = given().auth() .basic("username", "****").when() // enter
 * valid credentials .get("http://lms rest assured api/ "); // use valid api
 * 
 * try { // We used Jackson's ObjectMapper to read our JSON document into a tree
 * of JsonNode objects
 * 
 ...
 * 
 * CsvSchema.Builder csvSchemaBuilder = CsvSchema.builder(); JsonNode
